...
- **环境变量**：`.env` 提供 `VITE_APP_TITLE`/`VITE_ROUTER_MODE`/`VITE_REQUEST_TIMEOUT`；`.env.development` 有 `VITE_BASE_URL`/`VITE_BASE_API`；`.env.production` 有 `VITE_DROP_CONSOLE`/`VITE_DROP_DEBUGGER`。`index.html` 用 `%VITE_APP_TITLE%` 原生注入。`.env` 未 gitignore（仅公开变量）。
...
## 项目内 Skill
- `git-commit-msg`：依 git diff 生成中文 Conventional Commits message，只输出不擅自 commit；`.workbuddy/` 一并提交；message 仅单行 `type: 描述`。
- `path-alias`：`@/*` 别名完整配置流程。
- `add-global-component`：新增全局组件全流程。

## 工程化进度（截至 2026-07-22）
**已完成**：脚手架清理、路径别名、UnoCSS（含自定义 rules/shortcuts）、Pinia、vue-router（含 404 兜底）、unplugin 自动导入、SVG 图标 + SvgIcon、Vite 插件抽离、reset.css、StorageCache 封装、axios 请求层（实例 + 三拦截器 + ApiResponse 契约 + 类型收紧）、构建期 drop console/debugger、代码格式化 oxfmt、3 个项目 Skill、严格 TS 配置（noUnusedLocals/Parameters、erasableSyntaxOnly、tsconfig.node 用 verbatimModuleSyntax/nodenext）、多环境 .env（dev/prod）。
**未做（模板常见项）**：业务/示例页面（App.vue=`<router-view>`、Layout 仅占位）、路由 children/懒加载/路由守卫、Layout 实际布局（头部/侧边栏/菜单）、UI 组件库（Element Plus 明确不引入）、ESLint/husky/lint-staged、i18n、构建优化（chunk 分包/gzip/CDN）、单元测试（Vitest）、CI/CD、test/staging 多环境 .env。

...
- 状态：`src/store/index.ts` 导出 `createPinia()` 单例 + `setupStore(app)`；含 `store/modules/app.ts`、`store/modules/setting.ts`（主题/尺寸/动态标题）。
...
- `src/defaultSettings.ts`：`SystemSetting{theme:'light'|'dark', size:'default'|'small'|'large', showDynamicTitle:boolean}`(默认 `light`/`default`/`true`)。`size` 经 `App.vue` `<el-config-provider :size>` 全局生效；`showDynamicTitle` 经 `useDynamicTitle` watch 控制 `document.title`。
...
- `src/views/settings/index.vue`：主题开关 `:model-value="isDark"`+`@click` 复用 `useTheme().applyTheme` 做点击点圆形扩散（即时保存+动效）；尺寸/动态标题为预览式（`v-model` 绑 store 不落盘），由底部「保存设置」按钮 `saveSetting()` 统一落盘；「重置设置」→`resetSetting()`。移动端 `html[data-device='mobile']` 覆盖（行纵向堆叠、按钮满宽）。

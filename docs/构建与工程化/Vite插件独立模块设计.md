...
- `vite.config.ts` 极简，只声明用哪些插件，不关心插件怎么写。
- 每个插件可独立阅读、修改、调试。
- 插件函数可被其他 Node 配置（如预览 / 测试）复用。

## 工程化好处

- 构建配置与插件实现解耦，配置层与实现层职责清晰。
- `build/**` 纳入 `tsconfig.node.json`，插件代码享受 TS 检查（`noUnusedLocals` 等）。
- `nodenext` + 显式 `.ts` 后缀，避免扩展名推断歧义。

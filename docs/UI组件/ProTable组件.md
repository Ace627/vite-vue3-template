...
`ProTable` 是基于 Element Plus `el-table` 的轻量封装，解决「列表页重复书写 `el-table` + `el-table-column` 模板」的问题。底层仍是 `el-table`，封装只做列配置化、内容插槽化、原生能力全透传，不覆盖 `el-table` 自身能力。
...
- 未配 `slot` 时，直接用 `prop` 渲染行数据对应字段；透传给 `el-table-column` 的原生属性（`label` / `width` / `fixed` / `sortable` 等）照常生效。
- 配置了 `slot` 时，在 `el-table-column` 内渲染 `#default` 插槽，父组件用同名插槽自定义内容（如操作列、状态标签）。
...
- 移动端：当前 ProTable 未做移动端特殊处理；`ProPagination` 已内置 `isMobile` 适配。

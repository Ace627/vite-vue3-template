...
- 单一 `<SvgIcon>` 组件，禁止 EP 图标 / 字体图标（见 `../AGENTS.md`）。
- 运行时注入 sprite，无需逐个 import，按名渲染。
- `currentColor` 单色图标随主题变色；固定色可显式传 `color`。
- 新增图标只放文件 + 跑 `pnpm clean:svg`。

## 工程化好处

- 契合 `../AGENTS.md` 第 1、2 条——图标单一来源，组件图标位优先把 `<SvgIcon>` 放进插槽。
- 清理脚本自动适配 `currentColor`，团队无需手改 svg 属性。
- 给出「svg 图标 + 统一组件」的做法，新人照着接即可。
